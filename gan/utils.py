...
def interpolate_latent_space(gen, path):
    ...
    ##################################################################
    samples = torch.zeros(100, 128).to('cuda' if torch.cuda.is_available() else 'cpu')
    l = torch.linspace(-1, 1, 10)
    first, second = torch.meshgrid(l, l)
    samples[:, 0] = first.flatten()
    samples[:, 1] = second.flatten()
    generated = gen.forward_given_samples(samples)
