...
def compute_discriminator_loss(
    ...
    grad = torch.autograd.grad(discrim_interp, interp, grad_outputs=torch.ones_like(discrim_interp), create_graph=True)[0]
    loss_pt2 = lamb * torch.mean((torch.norm(grad.view(grad.shape[0], -1), dim=1)-1) ** 2)
    loss = loss_pt1 + loss_pt2
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
